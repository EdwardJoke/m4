...
pub fn build(b: *std.Build) void {
    ...
    // Pass version from build.zig.zon to the executable
    const version_opts = b.addOptions();
    version_opts.addOption([]const u8, "version", zon.version);
    exe.root_module.addImport("build_options", version_opts.createModule());

    // ── toolchain (mein) binary ──
    const mein_mod = b.createModule(.{
        .root_source_file = b.path("src/toolchain/main.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    b.installArtifact(mein_exe);

    const tool_step = b.step("tool", "Run the toolchain (mein)");
    const tool_cmd = b.addRunArtifact(mein_exe);
    tool_step.dependOn(&tool_cmd.step);
    tool_cmd.step.dependOn(b.getInstallStep());
    if (b.args) |args| {
        tool_cmd.addArgs(args);
    }

    // ── top-level run step (m4c) ──
    const run_step = b.step("run", "Run the m4c compiler");
