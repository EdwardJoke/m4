...
**Status:** Early development (v0.3.1). Core pipeline (scan → parse → type-check → compile → execute) is functional. QBE native compilation backend is functional. CLI UX polished — clean error messages without stack traces, `-h`/`--help` support, stdin piping for `lint`. Significant changes expected.
